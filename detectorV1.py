...
# Initialize RobotArm
robot_arm = RobotArm()
bottle_detected = False  # 新增變數來跟蹤水瓶是否被偵測到
    ...
    while viewer.is_available() and not exit_signal:
        if zed.grab(runtime_params) == sl.ERROR_CODE.SUCCESS:
            ...
            for i in range(0,obj_len):
                ...
                object_names.append(data[j+1])
                organized_data.append(data[j+2*obj_len])
                organized_data.ap
                pend(data[j+2*obj_len+1])
                j+=2
            # print("data", data)
